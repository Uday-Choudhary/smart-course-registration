...
const createSection=async (req,res) => {
    try {
        ...
        const section = await prisma.section.create({
            data:{
                sectionCode: sectionCode.trim(),capacity: parseInt(capacity),termId: parseInt(termId),},include: {
                term:true,},});

        res.status(201).json({
            success:true,message:"Section (Batch) created successfully",data:section,});
    } catch (error) {
